...
const DocumentEditor = ({ initialContent, onContentChange }) => {
  ...
  const handleDragEnd = (result) => {
    ...
    webSocketService.sendMessage(
      JSON.stringify({ params: 1, type: "REORDER", data: reorderedBlocks })
    );
  };
  ...
  const handleEditBlock = (id, newContent) => {
    ...
    // 수정된 블록을 서버에 동기화
    webSocketService.sendMessage(
      JSON.stringify({ params: 1, type: "UPDATE", data: updatedBlocks })
    );
  };
  ...
  const addBlock = () => {
    ...
    // 추가된 블록을 서버에 동기화
    webSocketService.sendMessage(
      JSON.stringify({ params: 1, type: "CREATE", data: newBlock })
    );
  };
  ...
  const deleteBlock = (id) => {
    ...
    // 삭제된 블록을 서버에 동기화
    webSocketService.sendMessage(JSON.stringify({ params: 1, type: "DELETE", data: id }));
  };
    ...
    if (event.key === "Enter") {
      ...
      if (isCursorAtStart) {
        ...
        if (currentBlock.content === "") {
          ...
          // 병합된 블록을 서버에 동기화
          webSocketService.sendMessage(
            JSON.stringify({ params: 1, type: "MERGE", data: updatedBlocks })
          );
        }
      }
    }
