...
const issueComment = {
    getComment: async (data) => {
        ...
        // API 호출
        return await apiCall(apiClient, "get", "/api/issue/comment/", { params: data.param });
    },
    ...
    editComment: async (data) => {
        ...
        // 유효성 검사
        validateParams(data.param, ["pid"]);
        if (!data.body) throw new Error("Comment body is required to edit a comment.");
    ...
    deleteComment: async (data) => {
        ...
        // API 호출
        return await apiCall(apiClient, "delete", "/api/issue/comment/delete/", { params: data.param });
    },
};
